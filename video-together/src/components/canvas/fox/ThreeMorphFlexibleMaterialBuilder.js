...
export default function ThreeMorphFlexibleMaterialBuilder(
  matParameters,
  rotationOrder
) {
  ...
  for (let iMorph = 0; iMorph < nMorphsAttribs; ++iMorph) {
    ...
    glslMorphJeelizCode +=
      "vec3 morphTargetJeeliz" +
      iAStr +
      " = morphJeelizRadius*(vec3(-1.,-1.,-1.) + 2.*floor(morphJeeliz" +
      iMorphStr +
      ")/morphJeelizPrecision);\n" +
      "vec3 morphTargetJeeliz" +
      iBStr +
      " = morphJeelizRadius*(vec3(-1.,-1.,-1.) + 2.*fract(morphJeeliz" +
      iMorphStr +
      "));\n" +
      "transformed += morphTargetJeeliz" +
      iAStr +
      " * morphJeelizInfluences[" +
      iAStr +
      "];\n" +
  ...
  vertexShaderSource = tweak_shaderAdd(
    ...
    "uniform mat4 modelMatrixDelayed;\n" +
      ...
      "uniform float morphJeelizInfluences[" +
      (2 * nMorphsAttribs).toString() +
      "];\n" +
