...
export default function ThreeMorphAnimGeomBuilder(specArg) {
  const lib_ajax = {
  ...
  lib_ajax.get(spec.url, function(data) {
    ...
    function duplicate_ifNecessary(vi, vti) {
      if (uviByVertexIndex[vi] === -1) {
        ...
        // we should duplicate the vertex
    ...
    geom.setAttribute(
      "position",
      new THREE.BufferAttribute(vertices32, 3, false)
    );
    ...
    // Construct morphs:
    console.log(
      "INFO in ThreeMorphAnimMeshBuilder: ",
      dataObj["morphs"].length,
      "morphs found"
    );
    ...
    for (
      let iMorphMultiplexed = 0;
      iMorphMultiplexed < _nMorphs / 2;
      ++iMorphMultiplexed
    ) {
      ...
      for (let i = 0; i < nVertices * 3; ++i) {
        const a = Math.round(
          (spec.morphPrecision * (1 + morphA[i] / morphRadius)) / 2
        ); // between 0 (-morphRadius) and 1 (maxRadius)
        const b = Math.round(
          (spec.morphPrecision * (1 + morphB[i] / morphRadius)) / 2
        ); // between 0 (-morphRadius) and 1 (maxRadius)
        morphMultiplexed[i] = a + b / spec.morphPrecision;
      }
      geom.setAttribute(
        "morphJeeliz" + iMorphMultiplexed.toString(),
        new THREE.BufferAttribute(morphMultiplexed, 3, false)
      );
    }
