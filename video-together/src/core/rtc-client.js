...
export default class RTCClient {
  constructor() {
    ...
    //this._clinet.enableCamera = false;
    this._handleEvents();
  ...
  setExternalMediaTrack(track, type) {
    return new Promise((resolve, reject) => {
      try {
        this._clinet.setExternalMediaTrack(track, type);
        //this._clinet.setExternalMediaTrack(null, 0);
        this._localCameraStream = track;
        resolve();
      } catch (err) {
        reject(err);
      }
  ...
  setDisplayLocalVideo(video, streamType = 1) {
    streamType == 1
      ? (video.srcObject = this._localCameraStream)
    ...
    if (this._autoSubscribe) {
      if (this._isAudioOnlyMode) {
        this.subscribeOnlyAudio(publisher.userId).then((code) => {
          if (this._callBack) {
            console.warn("onSubscribeResult", {
              userId: publisher.userId,
              code: code,
            });
            this._callBack("onSubscribeResult", {
              userId: publisher.userId,
              code: code,
            });
          }
  ...
  _publish() {
    return new Promise((resolve, reject) => {
      this._clinet
        ...
        .then(() => {
          if (
            !this._clinet.configLocalAudioPublish &&
            !this._clinet.configLocalCameraPublish &&
            !this._clinet.configLocalScreenPublish
          ) {
            this._isPublish = false;
            this._isMuteLocalCamera = false;
            this._isMuteLocalMic = false;
  ...
  _checkSubscribeState(userId) {
    ...
    let arr = streamConfigs.filter((v) => {
      return v.subscribed;
    });
    if (arr.length > 0) {
      return true;
  ...
  _getAuthInfo(channel, userName) {
    return new Promise((resolve, reject) => {
      Http.getAuthInfo(channel, userName)
        .then((authInfo) => {
  ...
  _executeAutoMute(data) {
    if (this._userMuteInfo[data.userId]) {
      ...
      data.streamConfigs.forEach((v) => {
        if (v.label == "sophon_audio") {
          if (this._userMuteInfo[data.userId].isAudioMute !== v.muted) {
            ...
            this._callBack("onUserAudioMuted", {
              userId: data.userId,
              mute: v.muted,
            });
          }
      ...
      // 加上 并判断muted 是否是true
      this._userMuteInfo[data.userId] = {
        isAudioMute: false,
        isVideoMute: false,
      };
      data.streamConfigs.forEach((v) => {
      ...
      if (this._userMuteInfo[data.userId].isAudioMute) {
        this._callBack("onUserAudioMuted", {
          userId: data.userId,
          mute: this._userMuteInfo[data.userId].isAudioMute,
        });
      }
    }
  }
  ...
  _executeVideoMute(data) {
    if (this._userMuteInfo[data.userId]) {
      ...
      data.streamConfigs.forEach((v) => {
        if (v.label == "sophon_video_camera_large") {
          if (this._userMuteInfo[data.userId].isVideoMute !== v.muted) {
            ...
            this._callBack("onUserVideoMuted", {
              userId: data.userId,
              mute: v.muted,
            });
          }
      ...
      if (this._userMuteInfo[data.userId].isVideoMute) {
        this._callBack("onUserVideoMuted", {
          userId: data.userId,
          mute: this._userMuteInfo[data.userId].isVideoMute,
        });
      }
    }
  }
